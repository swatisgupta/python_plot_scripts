...
def sync_mem_papi(dict, nths, indexVm, indexPapi):  
    i = 0
    
    all_marks = {}
    while i < nths:
        marks = []
        papi_keys = sorted(dict[i][indexPapi])
        mem_keys = sorted(dict[i][indexVm])
        papi_ar = numpy.asarray(papi_keys)
        for keys in mem_keys:
            if dict[i][indexVm][keys] != 0:
                index = numpy.argmax(papi_ar > keys)
                marks.append(index.tolist())
        all_marks[i] = marks
        i += 1
    return all_marks
...
def plot_memory_ravg(nprocs, plt, nrows, vals, running_avg, running_avg2, window, title):
    ...
    for i in range(nprocs):
        ...
        if j == 0:
            sp[row_cnt, j].set_ylabel('GigaBytes')
...
def plot_sync_mem_papi(nprocs, plt, nrows, vals, title):
    global max_len
    global max_len2
    ...
    for i in range(nprocs):
        ...
        sp[row_cnt, j].set_xlim([0, max_len2]);
        sp[row_cnt, j].set_ylim([0, max_len]);
        color=Colors[i]
        sp[row_cnt, j].plot(vals[i], label=labl, color=color)
        if j == 0:
            sp[row_cnt, j].set_ylabel('Last PAPI Sample collected')
...
def plot_papi_percentage(nprocs, plt, nrows, val_l, val_tot, title):
    ...
    for i in range(nprocs):
        ...
        sp[row_cnt, j].plot(l_p, color=color)
        j += 1
    save_plot(plt, title)
...
def main():
    ...
    sync_mp = sync_mem_papi(mylist, nprocs, unmap[RSS], unmap[L3])  
    ...
    for i in range(nprocs):
        ...
        scale=1000000
        val_vmhwm_sc.append([ j/scale for j in vals_vmhwm[i] ]);
        val_rss_sc.append([ j/scale for j in vals_rss[i] ]);
        #[j for k, j in zip(vals_tcyc[i][:-1], vals_tcyc[i][1:])])
        #val_st.append([j-k for k, j in zip(vals_st[i][:-1], vals_st[i][1:])])
        #val_tot2.append([j+k for k, j in zip(vals_st[i], vals_ld[i])])
        val_vmhwm.append(running_mean(val_vmhwm_sc[i])) 
        val_rss.append(running_mean(val_rss_sc[i])) 
        val_vmhwm2.append(running_mean2(val_vmhwm_sc[i], avg_window)) 
        val_rss2.append(running_mean2(val_rss_sc[i], avg_window)) 
        map = []
        val_vmhwm_ccnt.append(consecutive_count(val_vmhwm_sc[i], map)) 
        val_map_vmhwm.append(map)
        map = []
        val_rss_ccnt.append(consecutive_count(val_rss_sc[i], map)) 
        val_map_rss.append(map)
        if max_mem < max(val_vmhwm_sc[i]):
            max_mem = max(val_vmhwm_sc[i])

    #plot_memory(nprocs, plt, nrows, vals_vmhwm, "Virtual Memory High Water Mark")
    #plot_memory(nprocs, plt, nrows, keys_rss, "Resident Set Size Timestmaps")

    plot_memory_ravg(nprocs, plt, nrows, val_vmhwm_sc, val_vmhwm, val_vmhwm2, avg_window, "Virtual Memory High Water Mark")
    plot_memory_ravg(nprocs, plt, nrows, val_rss_sc, val_rss, val_rss2, avg_window, "Resident Set Size")
    plot_memory_boxplots(nprocs, plt, nrows, val_vmhwm_sc, "Boxplots: Virtual Memory High Water Mark")
    plot_memory_boxplots(nprocs, plt, nrows, val_rss_sc, "Boxplots: Resident Set Size")
    ...
    plot_sync_mem_papi(nprocs, plt, nrows, sync_mp, "PAPI counter corresponding to memory sample")
